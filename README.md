...
вид полиморфизма не используется из-за динамической типизации языка.
...
Рассмотрим полиморфизм подтипов подробнее:  
Рассмотрим пример на C#, языке с явной статической типизацией, чтобы продемонстрировать полиморфизм
подтипов:
```C#
using System;
...
// Дочерние классы
class Dog : Animal {
    public override void Speak() { // переопределяем метод с помощью override
...
class Program {
    static void Main() {
        ...
        // Вызов метода Speak для объектов разных классов
        animal1.Speak(); // Вывод: "Woof!"
        animal2.Speak(); // Вывод: "Meow!"
    }
}
```
В данном примере используется ключевое слово override для явного переопределения метода `Speak`, 
унаследованного от базового класса `Animal`

В языках же с динамической типизацией проявление полиморфизма подтипов можно наблюдать даже без 
использования наследования:
```Python
def plus_operation(a, b):
    return a + b

plus_operation(2, 2) # 4
plus_operation('Hello ', 'World') # Hello World
plus_operation(['a', 'b'], [1, 2]) # ['a', 'b', 1, 2]
```
То есть в языках с динамической типизацией полиморфизм подтипов позволяет использовать одну и ту же 
функцию или оператор для разных типов данных, при условии, что эти типы данных поддерживают 
соответствующие операции.

### Абстракция

Абстракция позволяет скрыть детали реализации и предоставить упрощенное представление объекта или 
концепциях. Простыми словами основная идея заключается в том, чтобы представить объект минимальным
набором методов и при этом с достаточной точностью для решаемой задачи. Понять парадигму абстракции 
...
# Создаем объекты с использованием абстракции
dog = Dog("Leya")
cat = Cat("Philin")

# Вызываем метод speak() на объектах разных классов
print(dog.speak())  # Вывод: "Leya says Woof!"
print(cat.speak())  # Вывод: "Philin says Meow!"
```
В этом примере Animal является абстрактным классом, который определяет общий интерфейс для животных. 
У него есть атрибут name и абстрактный метод speak(), который должен быть реализован в дочерних классах.
Dog и Cat - это конкретные реализации животных, которые наследуют от Animal. Они реализуют метод 
speak() в соответствии с конкретным видом животных.
Используя абстракцию, мы можем создавать объекты разных классов, но работать с ними через общий 
интерфейс speak(), не беспокоясь о деталях их реализации. Это помогает упростить код и сосредоточиться 
на важных характеристиках и поведении объектов.

Важно сказать, что в Python существует небольшой модуль `abc` для работы с абстрактными классами и 
методами:
```Python
from abc import ABC, abstractmethod

# Определение абстрактного класса с помощью наследования от ABC (Abstract Base Class)
class Shape(ABC):
    @abstractmethod
    def area(self):
        pass
...
# Создание подкласса, который реализует абстрактный метод
class Circle(Shape):
...
circle = Circle(5)
print("Площадь круга:", circle.area())  # Вывод: Площадь круга: 78.5
```

### Посылка сообщений

Посылка сообщений - это способ взаимодействия объектов в объектно-ориентированных языках программирования.
Она, как и другие парадигмы, может быть реализована по-разному в языках со статической и динамической 
типизацией.

В языках со статической типизацией типы данных определяются на этапе компиляции, т.е компилятор заранее 
понимает, какой тип объекта или переменной и какие методы можно вызвать.
В парадигме посылки сообщений в статически типизированных языках, объекты могут взаимодействовать только 
с теми методами, которые определены в их типах. Это называется "вызовом метода" и может быть статической 
(вызывается на этапе компиляции) или динамической (вызывается на этапе выполнения).
...
class Program {
    static void Main() {
        Dog myDog = new Dog(); // Создаем объект myDog типа Dog
        myDog.Bark(); // Вызов метода Bark() объекта myDog
    }
}
```
То есть для языков со статической типизацией парадигма "Посылка сообщений" это просто процесс, когда 
объект вызывает метод у другого объекта (за исключением С, там если объект не имеет вызываемого 
метода, то ничего не произойдет).

В языках же с динамической типизацией, таких как Python и JavaScript, типы данных переменных и объектов 
определяются на этапе выполнения. Это означает, что тип объекта может изменяться во время выполнения 
программы. В парадигме посылки сообщений в динамически типизированных языках, объекты могут 
взаимодействовать с методами, которые они поддерживают на текущем этапе выполнения.
```Python
class Dog:
    def bark(self):
        print("Woof!")
...
# Можем также динамически добавить новый метод
def greet(self):
    print("Hello!")

my_dog.greet = greet  # Добавление метода greet
my_dog.greet()  # Вызов нового метода greet()
```
### Повторное использование

Парадигма "Повторное использование" в Python подразумевает создание кода, который может быть многократно 
использован в разных частях программы или в разных программах.

В Python есть несколько способов достичь повторного использования кода:
1. Функции: Вы можете определять функции, которые выполняют конкретные задачи, и затем вызывать их в 
разных частях программы. Это уменьшает дублирование кода и повышает читаемость и обслуживаемость.
2. Классы и объекты: Вы можете создавать классы и объекты, которые инкапсулируют данные и методы. Это 
позволяет создавать экземпляры классов, которые могут быть многократно использованы в разных частях 
программы.
3. Модули и библиотеки: Вы можете создавать модули и библиотеки, которые содержат набор функций и 
классов для повторного использования в разных проектах. Это позволяет вам создавать собственные 
библиотеки и использовать сторонние библиотеки для решения различных задач.
4. Mixins: В Python вы можете использовать механизм наследования и миксинов для создания классов, 
которые могут наследовать функциональность от нескольких родительских классов. Это позволяет повторно 
использовать код, добавляя его к разным классам.
```Python
class JSONMixin:
    def to_json(self):
        import json
...
json_data = person.to_json()  # '{"name": "Alice", "age": 30}'
```
